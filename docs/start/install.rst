...
For inference, vllm 0.12.0 has been tested with mxfp4 models. When running training-time weight sharing set ``VLLM_ENABLE_V1_MULTIPROCESSING=0`` so the in-process ``model_executor`` handle remains available.
